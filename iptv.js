var WidgetMetadata = {
  ...
  description: "支持 EPG 的 M3U 直播源。支持 WebDAV 等 HTTP(S) 链接。",
  modules: [
    {
      id: "loadList",
      title: "全部频道",
...
async function searchChannels(params) {
  ...
  var filtered = channels.filter(function(c) {
    return c.title.toLowerCase().indexOf(kw) !== -1 || c.group.toLowerCase().indexOf(kw) !== -1;
  });
